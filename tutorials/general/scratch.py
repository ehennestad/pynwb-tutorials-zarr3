...
raw_path = "raw_data.nwb.zarr"
processed_path = "processed_data.nwb.zarr"
scratch_path = "scratch_analysis.nwb.zarr"

with NWBZarrIO(raw_path, mode="w", force_overwrite=True) as io:
    io.write(nwb)
...
with NWBZarrIO(processed_path, mode="w", manager=raw_io.manager, force_overwrite=True) as io:
    io.write(nwb_proc)
...
# .. [#]
#    .. note::
#       The ``processed_data.nwb.zarr`` store (i.e., our copy) stores our processing module and contains
#       external links to all data in our original store, i.e., the data from our raw store is being linked
#       to, not copied. This allows us to isolate our processing data in a separate store while still
#       allowing us to access the raw data from our ``processed_data.nwb.zarr`` store, without having to
#       duplicate the data.
#
...
proc_io = NWBZarrIO(processed_path, "r")
...
with NWBZarrIO(scratch_path, "w", manager=proc_io.manager, force_overwrite=True) as io:
...
scratch_io = NWBZarrIO(scratch_path, "r")
