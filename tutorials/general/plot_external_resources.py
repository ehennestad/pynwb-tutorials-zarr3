...
###############################################################################
# Write and read the NWB store
# ----------------------------
# Writing the store keeps the HERD inside it. Reading the store back makes the HERD available again
# through the ``external_resources`` field.

filename = "external_resources_tutorial.nwb.zarr"
with NWBZarrIO(filename, mode="w", force_overwrite=True) as io:
    io.write(nwbfile)

read_io = NWBZarrIO(filename, mode="r")
