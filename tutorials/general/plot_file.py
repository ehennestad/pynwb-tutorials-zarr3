...
# -------------------
#
# Writing of an NWB file is carried out using the :py:class:`~hdmf_zarr.nwb.NWBZarrIO` class [#]_,
# which writes the file as a Zarr v3 store instead of an HDF5 file.
#
# To write an :py:class:`~pynwb.file.NWBFile`, use the :py:meth:`~hdmf.backends.io.HDMFIO.write` method.

nwbfile_path = "basics_tutorial.nwb.zarr"

io = NWBZarrIO(nwbfile_path, mode="w", force_overwrite=True)
io.write(nwbfile)
io.close()

####################
# You can also use :py:class:`~hdmf_zarr.nwb.NWBZarrIO` as a context manager:

with NWBZarrIO(nwbfile_path, "w", force_overwrite=True) as io:
    io.write(nwbfile)
...
# -------------------
#
# As with writing, reading is also carried out using the :py:class:`~hdmf_zarr.nwb.NWBZarrIO` class.
# To read the store we just wrote, create another :py:class:`~hdmf_zarr.nwb.NWBZarrIO` object with the mode set
# to ``"r"``, and use the :py:meth:`~hdmf.backends.io.HDMFIO.read` method to retrieve an
...
# does not read the data values, but presents a Zarr array that can be indexed to read data.
# You can use the ``[:]`` operator to read the entire data array into memory.

with NWBZarrIO(nwbfile_path, "r") as io:
...
####################
# .. note::
#     If you use :py:class:`~hdmf_zarr.nwb.NWBZarrIO` as a context manager during read,
#     be aware that the :py:class:`~hdmf_zarr.nwb.NWBZarrIO` gets closed and when the
...
# ------------------------
#
# To append to a store, read it with :py:class:`~hdmf_zarr.nwb.NWBZarrIO` and set the ``mode`` argument to ``'a'``.
...
io = NWBZarrIO(nwbfile_path, mode="a")
nwbfile = io.read()
...
#
# .. [#] HDF5 is the primary backend supported by NWB; Zarr is an alternative backend provided by
#        :hdmf-zarr:`hdmf-zarr<>`. These tutorials use the Zarr backend throughout.
#
