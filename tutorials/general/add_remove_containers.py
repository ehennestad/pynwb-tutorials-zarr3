...
#
# 1. open the store with an :py:class:`~hdmf_zarr.nwb.NWBZarrIO` object in read/write mode
#    (``mode='r+'`` or ``mode='a'``)
# 2. read the :py:class:`~pynwb.file.NWBFile`
# 3. add container objects to the :py:class:`~pynwb.file.NWBFile` object
# 4. write the modified :py:class:`~pynwb.file.NWBFile` using the same
#    :py:class:`~hdmf_zarr.nwb.NWBZarrIO` object
#
# For example:
...
filename = "nwbfile.nwb.zarr"
with NWBZarrIO(filename, "w", force_overwrite=True) as io:
    io.write(nwbfile)

# open the NWB file in r+ mode
with NWBZarrIO(filename, "r+") as io:
...
# :py:meth:`ZarrIO.export <hdmf_zarr.backend.ZarrIO.export>` method, detailed below, to modify an NWB store
# in these ways.
#
# Exporting a written NWB file to a new file path
# -----------------------------------------------
# Use the :py:meth:`ZarrIO.export <hdmf_zarr.backend.ZarrIO.export>` method to read data from an existing NWB
# store,
...
export_filename = "exported_nwbfile.nwb.zarr"
with NWBZarrIO(filename, mode="r") as read_io:
    ...
    with NWBZarrIO(export_filename, mode="w", force_overwrite=True) as export_io:
...
with NWBZarrIO(export_filename, "r") as io:
    read_nwbfile = io.read()
    print(read_nwbfile)
...
# because the invalid name would otherwise be interpreted as nested Zarr groups and corrupt the store.
#
# The example below first synthesizes a legacy store by writing a :py:class:`~pynwb.device.Device` and
# then adding the string ``model`` attribute directly with the Zarr API (PyNWB cannot write a string
# ``model`` under the current schema). It then reads the store and replaces the read-only model.
...
filename = "legacy_device_model.nwb.zarr"
with NWBZarrIO(filename, "w", force_overwrite=True) as io:
    io.write(nwbfile)

# add the legacy Device.model string attribute that schema versions before 2.9 used. hdmf-zarr writes
# consolidated metadata, so the store's consolidated view has to be rebuilt after editing the attribute
# for the change to be visible on read.
root = zarr.open_group(filename, mode="r+", use_consolidated=False)
root["general/devices/my_device"].attrs["model"] = "MFC_200/250-0.66_40mm_MF2.5:FLT"
...
#
# The legacy store keeps ``model`` as a string attribute, so it cannot be modified in place
# (``mode="r+"``) or written to a new path with :py:meth:`~hdmf_zarr.backend.ZarrIO.write`. Use
# :py:meth:`~hdmf_zarr.backend.ZarrIO.export` instead, and call
...
with NWBZarrIO(filename, mode="r") as read_io:
    ...
    export_filename = "upgraded_device_model.nwb.zarr"
    with NWBZarrIO(export_filename, mode="w", force_overwrite=True) as export_io:
        export_io.export(src_io=read_io, nwbfile=read_nwbfile)
...
###############################################################################
# For more information about the export functionality, see the hdmf-zarr documentation for
# :py:meth:`ZarrIO.export <hdmf_zarr.backend.ZarrIO.export>`.
#
