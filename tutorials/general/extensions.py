...
#
# Now that we have some data, lets write our store. You can choose not to cache the spec by setting
# cache_spec=False in :py:meth:`~hdmf_zarr.backend.ZarrIO.write`

from hdmf_zarr.nwb import NWBZarrIO

io = NWBZarrIO("cache_spec_example.nwb.zarr", mode="w", force_overwrite=True)
io.write(nwbfile)
io.close()
...
# the store. To disable this, set ``load_namespaces=False`` in the
# :py:class:`~hdmf_zarr.nwb.NWBZarrIO` constructor.
#
...
with NWBZarrIO("test_multicontainerinterface.nwb.zarr", "w", force_overwrite=True) as io:
    io.write(nwbfile)
...
with NWBZarrIO("test_multicontainerinterface.nwb.zarr", "r") as io:
    nwb = io.read()
...
from hdmf_zarr.nwb import NWBZarrIO  # noqa: F811
...
with NWBZarrIO("test_cortical_surface.nwb.zarr", "w", force_overwrite=True) as io:
    io.write(nwbfile)
