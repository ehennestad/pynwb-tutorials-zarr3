...
# :py:class:`~hdmf_zarr.backend.ZarrIO` or :py:class:`~hdmf_zarr.nwb.NWBZarrIO`) then
...
# .. tip::
#
#    Both the HDF5 and the Zarr I/O backends process iterative data writes one-dataset-at-a-time.
#    This means, that
...
def write_test_file(filename, data, close_io=True):
    ...
    :param close_io: Close and destroy the NWBZarrIO object used for writing (default=True)

    :returns: None if close_io==True otherwise return NWBZarrIO object used for write
    """
    ...
    io = NWBZarrIO(filename, "w", force_overwrite=True)
    io.write(nwbfile)
    if close_io:
        io.close()
        del io
...
write_test_file(filename="basic_iterwrite_example.nwb.zarr", data=data)
...
class SparseMatrixIterator(AbstractDataChunkIterator):
    ...
    def __next__(self):
        ...
        location within the matrix. Chunks are non-overlapping. REMEMBER: the storage backend does not support all
...
#####################
# To control compression, chunking, and other dataset I/O settings we wrap our data via
# :py:class:`~hdmf_zarr.utils.ZarrDataIO`, the Zarr analogue of
# :py:class:`~hdmf.backends.hdf5.h5_utils.H5DataIO`. Under Zarr v3, a compressor is a
# :py:class:`zarr.abc.codec.BytesBytesCodec` from :py:mod:`zarr.codecs` rather than a
# compression name plus options.
#
# .. important::
#
...
# Create our sparse matrix data, opting out of Zarr's default compressor
...
# Increase the chunk size, still without compression
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# Here we simply use our wrapped ``SparseMatrixIterator`` as input for our ``TimeSeries``

write_test_file(filename="basic_sparse_iterwrite_example.nwb.zarr", data=data)
write_test_file(filename="basic_sparse_iterwrite_compressed_example.nwb.zarr", data=data2)
write_test_file(filename="basic_sparse_iterwrite_largechunks_example.nwb.zarr", data=data3)
write_test_file(
    filename="basic_sparse_iterwrite_largechunks_compressed_example.nwb.zarr", data=data4
)
...
# ^^^^^^^^^^^^^^^^^
#
# Now lets check out the size of our data store and compare it against the expected full size of our matrix.
# A Zarr store is a directory of chunk files, so its size is the sum of the files it contains rather than
# the size of a single file.
import os


def store_size(path):
    """Return the total size in bytes of every file in a Zarr store directory."""
    total = 0
...
file_size = store_size("basic_sparse_iterwrite_example.nwb.zarr")  # Real size of the store
file_size_compressed = store_size("basic_sparse_iterwrite_compressed_example.nwb.zarr")
file_size_largechunks = store_size("basic_sparse_iterwrite_largechunks_example.nwb.zarr")
file_size_largechunks_compressed = store_size(
...
print("2) NWB Zarr store (no compression):")
print("   Store Size    :  %.2f MB" % (file_size / mbfactor))
print("   Reduction     :  %.2f x" % (expected_size / file_size))
print("3) NWB Zarr store (with GZIP compression):")
print("   Store Size    :  %.5f MB" % (file_size_compressed / mbfactor))
print("   Reduction     :  %.2f x" % (expected_size / file_size_compressed))
print("4) NWB Zarr store (large chunks):")
print("   Store Size    :  %.5f MB" % (file_size_largechunks / mbfactor))
print("   Reduction     :  %.2f x" % (expected_size / file_size_largechunks))
print("5) NWB Zarr store (large chunks with compression):")
print("   Store Size    :  %.5f MB" % (file_size_largechunks_compressed / mbfactor))
...
# ^^^^^^^^^^
#
# * **1) vs 2):** While the full matrix would have a size of ``8TB`` the Zarr store is only about ``0.89MB``. This is
#   roughly the same as the real occupied size of ``0.8MB``. When using chunking, Zarr only writes chunk files that
#   actually contain data. In (2) the size of our chunks align perfectly with the occupied chunks of our sparse
#   matrix, hence, only the minimal number of chunks is written. A slight overhead (here about 0.09MB) is expected
#   because our store also contains the additional objects from the NWBFile, plus one ``zarr.json`` metadata file
#   per array and group.
# * **3) vs 2):** Adding compression does not yield any improvement here -- the store even grows slightly. This is
#   expected, because, again we selected the chunking here in a way that we already write the minimum amount of
#   storage to represent our data and lossless compression of random data is not efficient.
# * **4) vs 2):** When we increase our chunk size to ``(100,100)`` (i.e., ``100x`` larger than the chunks produced by
#   our matrix generator) we observe an accordingly roughly ``100x`` increase in store size (about ``80MB``). This is
#   expected since our chunks now do not align perfectly with the occupied data and each touched chunk is written in
#   full, fill values included.
# * **5) vs 4):** When using compression for the larger chunks we see a significant reduction
#   in store size (``1.17MB`` vs. ``80MB``). This is because the written chunks now contain in addition to the random
#   values large areas of constant fill values, which compress easily.
#
#   This is also why Zarr's default of always compressing matters: had we left the default Zstd compressor in place
#   for (4), it would have come out at under ``1MB`` and the effect of the chunk size would have been invisible.
#
# **Advantages:**
#
# * We only need to hold one :py:class:`~hdmf.data_utils.DataChunk` in memory at any given time
# * Only the chunks of the Zarr store that contain non-default values are ever written
# * The overall size of our store is reduced significantly
# * Reduced I/O load
...
#   .. code-block:: python
#
#       io = NWBZarrIO('basic_sparse_iterwrite_example.nwb.zarr', 'r')
#       nwbfile = io.read()
...
#    array. Whichever backend is used, we need to take care that the selection we yield can be understood
#    by that backend (h5py for HDF5, zarr-python for Zarr).
...
write_test_file(filename="basic_sparse_iterwrite_largearray.nwb.zarr", data=data)

####################
# .. tip::
#
#       Again, if we want to explicitly control how our data will be chunked (compressed etc.)
#       in the Zarr store then we need to wrap our :py:class:`~hdmf.data_utils.DataChunkIterator`
#       using :py:class:`~hdmf_zarr.utils.ZarrDataIO`
...
# Read the NWB store
with NWBZarrIO("basic_sparse_iterwrite_largearray.nwb.zarr", "r") as io:
    nwbfile = io.read()
...
write_test_file(filename="basic_sparse_iterwrite_multifile.nwb.zarr", data=data)
...
#    * The selection for the :py:class:`~hdmf.data_utils.DataChunk` is not supported by the storage
#      backend (e.g., unordered lists etc.)
#
#    Other common mistakes:
#
#    * Choosing inappropriate chunk sizes. This typically means bad performance with regard to I/O and/or storage cost.
#    * Using auto chunking without supplying a good recommended_data_shape. Automatic chunking can only make a
#      good guess of what the chunking should be if it (at least roughly) knows what the shape of the array
#      will be.
...
#      target dataset. This can result in slow I/O performance, for example, when each chunk of a dataset
#      needs to be updated multiple times on write. For example, when using compression this would mean that
#      the backend may have to read, decompress, update, compress, and write a particular chunk each time it is
#      being updated.
#
#
...
# ------------------------------------------------
#
# The upstream HDF5 version of this tutorial ends with a section that allocates an empty dataset up
# front with :py:class:`~hdmf.backends.hdf5.h5_utils.H5DataIO` (using its ``shape``, ``dtype`` and
# ``maxshape`` arguments), keeps the file open, and then fills the dataset in afterwards through
# ``dataio.dataset``.
#
# That section is omitted here because it has no Zarr equivalent:
# :py:class:`~hdmf_zarr.utils.ZarrDataIO` accepts only ``data``, ``chunks``, ``fillvalue``,
# ``compressor``, ``filters`` and ``link_data`` -- there is no way to declare a dataset by ``shape``
# and ``dtype`` alone and allocate it empty, and a Zarr array has no ``maxshape``, since any Zarr
