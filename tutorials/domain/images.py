...
nwbfile_path = os.path.abspath("images_tutorial.nwb.zarr")
moviefiles_path = [
...
#
# Image data can be stored either in the NWB store or as an external image file.
...
# the experiment. Image data can be stored either in the NWB store or as an external image file.
# When color images are stored in the NWB store the color channel order is expected to be RGB.
#
...
# resolve the example image relative to this script so the tutorial runs from any working directory
logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "assets", "logo_pynwb.png")
img = Image.open(logo_path)  # an example image
...
# ---------------------------------------
# Write the file as a Zarr v3 store with :py:class:`~hdmf_zarr.nwb.NWBZarrIO`.
...
# --------------------------
#
# To read the NWB store, use another :py:class:`~hdmf_zarr.nwb.NWBZarrIO` object,
# and use the :py:meth:`~hdmf_zarr.nwb.NWBZarrIO.read` method to retrieve an
...
# does not read the data values into memory, but returns a Zarr array that can be indexed to read data.
