...
% and its entry is therefore stale.
    ...
    % Generated type classes go into a scratch folder that is emptied first. Several of
    % these stores embed extension schemas (mylab, ecog, test_multicontainerinterface),
    % whose classes exist only once they are generated from the store being read. Reusing
    % a populated folder -- or a MatNWB checkout that still holds classes generated by
    % earlier work -- lets those stores appear to read on a machine where they would fail
...
%
% Deliberately not passing "ignorecache": that flag skips generating classes from the
% schema cached in the store, which is the only source for the extension types some of
% these stores contain.
    try
        nwb = nwbRead(storePath, savedir=classDirectory); %#ok<NASGU>
        passed = true;
        message = "";
    catch cause
        passed = false;
